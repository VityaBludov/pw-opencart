...
const deleteAdminsQuery = `DELETE FROM user u WHERE u.email LIKE \'%@${urls.testData.emailDomain}\' AND u.date_added > DATE_SUB(NOW(), INTERVAL 1 DAY);`
...
teardown('delete test admin users from database', async () => {
    const deleteResult = await mysqlQuery(deleteAdminsQuery)
