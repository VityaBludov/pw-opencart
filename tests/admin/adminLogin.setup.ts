...
setup('login as product admin for tests', async ({ apiPage }) => {
    process.env.TOKEN_ADMIN_PRODUCT = await apiPage.login(defaultUsers.admin.productAdmin.username, defaultUsers.admin.productAdmin.password, authFileProductAdmin)
})
