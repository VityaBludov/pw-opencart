...
import { config, RandomUser } from '../resources/helper'
import fs from 'fs'
...
setup('register user for tests', async ({ request }) => {
    const pageResponse = await request.get(config.address.registration)
    ...
    const postResponse = await request.post(`${config.endpoint.registration}${registerToken}`, {
        form: {
