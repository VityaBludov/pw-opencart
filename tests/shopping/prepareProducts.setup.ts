...
const getProductsQuery = 'SELECT pd.meta_title FROM product p LEFT JOIN product_description pd ON p.product_id = pd.product_id WHERE p.quantity > 0 AND p.`status` = 1 LIMIT 1000;'
